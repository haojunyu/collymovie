...
func main() {
	...
	threadCollector.SetProxyFunc(rp)
	mailCollector.SetProxyFunc(rp)

	// Before making a request print "Visiting ..."
	threadCollector.OnRequest(func(r *colly.Request) {
		fmt.Println("Visiting", r.URL.String())
	})

	// Set error handler
	threadCollector.OnError(func(r *colly.Response, err error) {
		fmt.Println("Request URL:", r.Request.URL, "failed with response:", r, "\nError:", err)
	})

	// 查看请求结果
	threadCollector.OnResponse(func(r *colly.Response) {
